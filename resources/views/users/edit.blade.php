...
<section class="page-header" aria-labelledby="page-title">
    <h1 id="page-title">Edit Staff Account</h1>
    <p>Update account details for {{ $user->name }}</p>
</section>

<article class="form-container">
    <form action="{{ route('users.update', $user->id) }}" method="POST" aria-label="Edit staff account form">
        @csrf
        @method('PATCH')
        ...
        <div class="form-section">
            <div class="form-group @error('name') error @enderror">
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" required aria-required="true">
                @error('name')
            ...
            <div class="form-group @error('email') error @enderror">
                ...
                <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" required aria-required="true">
                @error('email')
                    <span class="form-error" role="alert">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group @error('username') error @enderror">
                ...
                <input type="text" name="username" id="username" value="{{ old('username', $user->username) }}" required aria-required="true">
                @error('username')
                    <span class="form-error" role="alert">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="form-section">
            ...
            <div class="form-group @error('password') error @enderror">
                ...
                @error('password')
                    <span class="form-error" role="alert">{{ $message }}</span>
                @enderror
                <p class="form-text">Minimum 8 characters if changing password.</p>
            </div>
        </div>
